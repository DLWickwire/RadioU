...
export default function ApplicationList() {
    //this keeps all my application data and loading stuff
    ...
    async function fetchApplications() {
        try {
            ...
            const appsWithStation = await Promise.all(
                // makes sure all the fetches from the radio api are done before finishing
                data.map(async (app) => {
                    const res = await fetch(
                        //loops all my apps and then sends a fetch to the radio api and returns an aray with an object
    ...
    //  this is too delete an application by ID and also pops up a warning before letting me delete
    async function handleDelete(id) {
        ...
        if (error) {
            //doesnt let me fully delete until after I click "ok " in the warning box
        ...
        <div className="list-container">
            ...
            {applications.length === 0 ? ( // checks the list of apps and if its empty it says "no applications found"
                ...
                <ul>
                    {applications.map(
                        (
                            app //map of all the applications currently submitted and the key makes it so i can track all the items from the tables
                        ) => (
                            <li key={app.id}>
                                <strong>{app.band_name}</strong> ({app.genre})
                                <br />
